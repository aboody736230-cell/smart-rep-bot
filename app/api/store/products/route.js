...
export async function GET() {
  const response = await supabaseRequest('?select=id,name,price,price_text,image_url,description,affiliate_url,store,category,branch,status&status=eq.published&order=created_at.desc&limit=500');
