...
export async function GET() {
  const response = await supabaseRequest('?select=id,name,price,image_url,description,affiliate_url,store,category,branch,status&company_id=eq.1&status=eq.published&order=created_at.desc&limit=100');
