...
const categories = [
  ...
  { id: 'clothes', name: 'الملابس', icon: '👗', subcategories: ['فساتين', 'بلوزات', 'فساتين سهرات', 'جينزات', 'ملابس داخلية', 'سراويل', 'معطف', 'جاكيت', 'تنوره'] },
...
const childSubcategories = {
  مواليد: [],
  بناتي: ['فساتين', 'ملابس داخلية', 'أحذية', 'كماليات', 'بناطيل', 'تيشرتات', 'قمصان', 'بجام', 'طقومات'],
  ولادي: ['أطقم', 'ملابس داخلية', 'أحذية', 'تيشرتات', 'بناطيل', 'قمصان'],
};
...
export default function Home() {
  ...
  const availableStores = ['SHEIN', 'Amazon', 'AliExpress'];
  const activeProducts = activeCategory ? sortProducts(publishedProducts.filter((product) => product.category === activeCategory.name && (!activeSubcategory || product.branch === activeSubcategory) && (!activeStore || product.store === activeStore))) : [];
  ...
  }).filter((product) => !activeStore || product.store === activeStore));
  ...
  return (
    <main className="store-shell" dir="rtl">
      <div className="store-container">
        ...
        <div className="toolbar-row"><div className="currency-bar"><span>العملة</span><div className="currency-switch" aria-label="تحويل العملة"><button type="button" className={currency === 'SOURCE' ? 'active' : ''} onClick={() => setCurrency('SOURCE')}>الأصلي</button><button type="button" className={currency === 'SAR' ? 'active' : ''} onClick={() => setCurrency('SAR')}>ر.س</button><button type="button" className={currency === 'USD' ? 'active' : ''} onClick={() => setCurrency('USD')}>$</button></div></div><label className="sort-box"><select value={sortOrder} onChange={(event) => setSortOrder(event.target.value)} aria-label="ترتيب المنتجات حسب السعر"><option value="default">السعر: الترتيب الافتراضي</option><option value="price-asc">السعر: من الأقل إلى الأعلى</option></select></label></div>
        ...
        <section ref={kidsSectionRef} className="kids-store-section" aria-label="قسم الأطفال">
          ...
          <div className="kids-main-branches">{['مواليد', 'بناتي', 'ولادي'].map((branch) => <button type="button" key={branch} className={`kids-main-branch ${activeChildBranch === branch ? 'active' : ''}`} onClick={() => { setActiveChildBranch(activeChildBranch === branch ? '' : branch); setActiveSubcategory(''); setActiveStore(''); }}>{branch}</button>)}</div>
          {activeChildBranch && childSubcategories[activeChildBranch] && <div className="kids-subcategory-list kids-store-subcategories"><span>اختيارات {activeChildBranch}</span>{childSubcategories[activeChildBranch].map((subcategory) => <button type="button" key={subcategory} className={`subcategory-pill ${activeSubcategory === subcategory ? 'subcategory-pill-active' : ''}`} onClick={() => { setActiveSubcategory(activeSubcategory === subcategory ? '' : subcategory); setActiveStore(''); }}>{subcategory}</button>)}</div>}
          {activeChildBranch && <div className="store-filter-row kids-store-filter" aria-label="فلترة منتجات الأطفال حسب المتجر"><span>المتجر:</span>{availableStores.map((store) => <button type="button" key={store} className={activeStore === store ? 'store-filter-button active' : 'store-filter-button'} onClick={() => setActiveStore(activeStore === store ? '' : store)}>{store === 'AliExpress' ? 'علي إكسبرس' : store}</button>)}</div>}
